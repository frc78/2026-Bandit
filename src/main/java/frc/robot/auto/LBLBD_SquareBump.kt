...
object LBLBD_SquareBump : Auto {
    ...
    private val autoStartPoint by FieldLocation(3.36, 5.737, Rotation2d((0).degrees))
    private val neutralZone by FieldLocation(5.15, 5.6, Rotation2d((0).degrees))
    private val leftOfFuel by FieldLocation(8.1, 7.0, Rotation2d((-90).degrees))
    private val fuel by FieldLocation(8.1, 4.8, Rotation2d((-90).degrees))
    ...
    override fun run() {
        ...
        when (step) {
            ...
            Step.SecondCrossBump -> {
                if (SwerveDrivetrain.driveToPose(autoStartPoint, 1.feet, alwaysMaxSpeed = true)) {
                    step++
                }
            }
